...
4. **XML Duplication**: *How do you load two identical robots without naming collisions in MuJoCo?* (Since MuJoCo MJCF files require unique names for all bodies, joints, geoms, and sites, we write a Python pre-processor script that parses the official XML via `ElementTree`, recursively duplicates the arm's base body tree, and appends `_left` and `_right` suffixes to all name attributes, while leaving mesh and material assets shared).
